...
# >>> НОВАЯ ФУНКЦИЯ: ОТПРАВКА УВЕДОМЛЕНИЯ <<<
...
def student_menu():
    ...
    # Рассчитываем общую стоимость завтрака и обеда
    breakfast_price = 0.0
    lunch_price = 0.0
    if menu_set:
        # Завтрак
...
def student_payment():
    ...
    if request.method == 'POST':
        ...
        # Находим самую позднюю дату окончания среди активных (end_date >= сегодня)
        ...
        # Определяем, от какой даты начинать новый период
        start_from = today
        if current_end_row:
        ...
        # Создаём новый абонемент (старые остаются в истории)
        db.execute('''
    ...
    # Получаем самый поздний активный абонемент (не истёкший)
